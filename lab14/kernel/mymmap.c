...
static int mymmap_mmap(struct file *file, struct vm_area_struct *vma)
{
	...
	if(len > my_data->size) return -EINVAL;
	...
	if(remap_pfn_range(vma, vma->vm_start, pfn, len, vma->vm_page_prot)) return -EAGAIN;
	...
	while(len > 0){
		...
		if(remap_pfn_range(vma, start, pfn, PAGE_SIZE, vma->vm_page_prot)) return -EAGAIN;
		start +=PAGE_SIZE;
...
static int mymmap_init(void)
{
	...
	page_area =(void*)page_address(page);
	area = page_area;
