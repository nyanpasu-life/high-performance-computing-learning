...
int array_length = DEFAULT_ARRAYLENGTH;
...
supervisor (supervisor_t *sup) {
  ...
  for (i = 0; i < iterations; i++) {
    ...
    int * input_arr = malloc(sizeof(int)*array_length);
    int * output_arr = malloc(sizeof(int)*array_length);
    for(int i=0;i<array_length;i++) input_arr[i] = rand()%MAX_NUM;

    if (write(sup->sockfd, (void*)&array_length, sizeof(int)) <=0) goto quit;
    if (shutting_down) goto quit;
    
    if (read (sup->sockfd, (void*)&ACK, sizeof(int)) <=0) goto quit;
    if (shutting_down) goto quit;

    if (write(sup->sockfd, (void*)input_arr, sizeof(int) * array_length ) <=0) goto quit;
    if (shutting_down) goto quit;

    if (read (sup->sockfd, (void*)output_arr, sizeof(int) * array_length ) <=0) goto quit;
    if (shutting_down) goto quit;
 ...
 quit:
  ...
  //int a = (int)sup->sockfd;
  //fprintf (stderr, "use sockfd : %d", a);
...
set_array_length () {
  char * p;
  if ((p = getenv ("ARRAY_LENGTH"))) {
    sscanf (p, "%d", &array_length);
  }
}
...
main(int argc, char **argv) {
   ...
   set_iterations ();
   set_array_length();
   fprintf (stderr, "driver: (version 2) concurrency = %d; iterations = %d; array length = %d\n", concurrency, iterations, array_length); 
   setalrmhandler ();
